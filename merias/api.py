...
def stock_entry(doc, method):
	...
	if doc.purpose == "Material Transfer":
		for d in doc.get('items'):
			blocked_qty = frappe.db.sql('''SELECT sum(smi.qty) FROM `tabSales Order Item` smi
			LEFT JOIN `tabSales Order` so ON smi.item_code = %s and so.name = smi.parent and
			smi.warehouse = %s and smi.is_blocked = 1 and so.status <> 'Cancelled' ''',
...
def check_availability_for_items_based_on_booked(doc, method):
	...
	for d in doc.get('items'):
		...
		bloked_qty = frappe.db.sql('''SELECT sum(smi.qty) FROM `tabSales Order Item` smi
		LEFT JOIN `tabSales Order` so ON smi.item_code = %s and so.name = smi.parent and
		smi.warehouse = %s and smi.is_blocked = 1 and so.status <> 'Cancelled' ''',
...
def si_for_items_based_on_booked(doc, method):
	...
	for d in doc.get('items'):
		if d.warehouse:
			...
			bloked_qty = frappe.db.sql('''SELECT sum(smi.qty) FROM `tabSales Order Item` smi
			LEFT JOIN `tabSales Order` so ON smi.item_code = %s and
			smi.is_blocked = 1 and so.status <> 'Cancelled' ''',
			(d.item_code)
			)
			actual_qty = frappe.db.sql("select sum(actual_qty) from `tabBin` \
